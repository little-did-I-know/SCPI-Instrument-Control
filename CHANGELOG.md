...
- Connections: a response that is nothing but a terminator is now a timeout on BOTH transports. `read()`/`query()` against a bare `\n` used to return `""`; they now log the discarded stray byte, keep reading for the real answer, and raise `SiglentTimeoutError` with the session marked out of step so the next send clears it. A lone terminator is the leftover of an earlier exchange, not an answer to the question just asked — returning it as `""` is how one late newline shifted every later answer by one query. Code that read `""` as "the instrument had nothing for me" must catch `SiglentTimeoutError` instead. (A response that is empty after whitespace but not terminator-only — e.g. `" \n"` — still comes back as `""`.)
