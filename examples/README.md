...
| `capture_pipeline.py` | End-to-end capture-to-report via `run_capture_pipeline`: a single-run capture with a passing `CriteriaSet`, and a batch capture (3 triggers, 2 channels) with a `CriteriaSet` where one channel fails, showing both the PASS and FAIL report paths. | None -- runs against the built-in mock scope; `--host <ip>` for real hardware |
