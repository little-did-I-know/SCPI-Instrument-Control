...
def test_ai_content_transfer_preserves_attribution(tmp_path):
    """Regression for main_window.py's _build_report(): it used to build a BRAND-NEW
    TestReport and copy only the AI-generated TEXT out of a dict, discarding the
    object the producer set findings_source/recommendations_source on (Task 4
    review, C1). That transfer now happens exclusively through
    TestReport.apply_ai_content(), which sets each text field and its `*_source`
    flag together so they cannot diverge. Drive that method directly (no Qt import
    needed, so this runs in CI) and check the rendered markdown carries the label."""
    from scpi_control.report_generator.llm.analyzer import ReportAnalyzer
    ...
    # Mirrors main_window.py's _build_report(): a fresh TestReport, with the
    # AI content dict applied the same way _build_report() does it now.
    report_b = TestReport(metadata=report_a.metadata)
    report_b.apply_ai_content(ai_content)

    assert report_b.findings_source == SUMMARY_SOURCE_AI
