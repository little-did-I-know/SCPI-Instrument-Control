...
| `AC` trigger coupling | legacy, modern, lecroy | Not supported on Tektronix: neither the TBS1000C (`DC`\|`HFRej`\|`LFRej`\|`NOISErej`, TBS p.151) nor the 2 Series MSO (`DC`\|`HFRej`\|`LFRej`\|`NOISErej`, MSO2 p.2-661) command set has an AC trigger-coupling token. Setting `scope.trigger.coupling = "AC"` (or `TriggerCoupling.AC`) on a Tektronix-dialect scope raises `FeatureNotSupportedError`; check `scope.capabilities.trigger_couplings` before setting it if the connected dialect isn't known ahead of time. |
