...
- **Plot annotations in reports.** Waveform, region, FFT and comparison-overlay
  ...
  **Annotate…** editor in the report generator app or from Python
  ...
  `examples/report_annotations.py`, and `examples/report_annotations_advanced.py`
  for the surfaces beyond a single plot: all four plot types, per-annotation
  colour/size/arrow overrides against the `PlotStyle` defaults, a region zoom
  inheriting clipped annotations, the sidecar round-trip, and the anchor list
  behind the **Annotate…** dialog. The README and the annotation section of the
  report-generator guide now carry a GIF of the API in use, generated by
...
- `WaveformRegion.markers`, an unused field that was serialized but rendered by
  nothing. Use `PlotAnnotation` instead.
...
- The hero GIF now also appears on the documentation landing page and in `examples/README.md`, not only the root README. Same asset, so there is nothing extra to regenerate.
...
- The README hero GIF now tours four examples running with no instrument attached — `basic_usage.py`, `math_channels.py`, `measurement_badges_example.py` and `screen_capture_example.py` — instead of showing a single waveform capture. Every frame's text is captured by running the example as a subprocess at generation time, so the image cannot drift from the code. The file keeps its path (`docs/images/mock-demo.gif`), so the README's `<img>` source is unchanged; only its `alt` text was updated to describe the new content.

### Fixed

- Docs: the landing page's Quick Example called `scope.measure.frequency()` and `scope.measure.vpp()`, neither of which exists — anyone copying it got an `AttributeError` on the last two lines. It now uses `scope.measurement.measure_frequency()`, and runs against the built-in mock with no instrument attached, matching the README's quick start. Verified by executing the block as written.
- Serial protocol decoders (I2C/SPI/UART): decoding could fabricate phantom events — bytes that were never on the wire — when a frame began too close to the end of a captured buffer. `_sample_at_time` resolved an out-of-range sample time to the nearest captured sample instead of rejecting it, so a truncated frame decoded exactly like a fully-captured one, indistinguishable from a genuine decode at every call site. This affected real hardware captures as much as the mock. Truncated frames are now dropped instead of decoded, so a decoder may report *fewer* events for the same capture than before — there is no migration path to describe, since anyone relying on the old behavior was relying on fabricated data. `examples/protocol_decoding.py`'s documented UART event summary changes from `{'DATA': 1}` to `{}` accordingly.
...
- Connections: `read_raw()` takes a `framing` argument (`Framing.AUTO` by default, so existing calls are unchanged). Custom `BaseConnection` subclasses must accept it (importable as `from scpi_control.connection import Framing`).
- Connections: a response that is nothing but a terminator is now a timeout on `read()` on both transports, and on `query()` on sockets. `read()` against a bare `\n` — or `b"\x00\n"`, a NUL byte then a terminator — used to return `""`; it now logs the discarded stray byte, keeps reading for the real answer, and raises `SiglentTimeoutError` with the session marked out of step so the next send clears it. `SocketConnection.query()` inherits the fix because it calls `read()`; `VISAConnection.query()` does not, because it delegates straight to `pyvisa`'s own `query()` rather than this module's `read()`, so a terminator-only answer there still comes back as `""`. A lone terminator is the leftover of an earlier exchange, not an answer to the question just asked — returning it as `""` is how one late newline shifted every later answer by one query. Code that read `""` as "the instrument had nothing for me" must catch `SiglentTimeoutError` instead, except when going through `VISAConnection.query()`. (A response that is empty after whitespace but not terminator-only — e.g. `" \n"` — still comes back as `""`.)
...
- Power supply: on the SPD3303X/SPD3303X-E, `output3.voltage`, `output3.current`, `output3.measure_*()`, `output3.get_mode()`, `output3.timer_enabled`, `output3.waveform_enabled` and reading `output3.enabled` now raise `FeatureNotSupportedError`. CH3 is the DIP-switch-selected fixed rail: its voltage is chosen on the front panel (QS0503X-E01B p.21) and the command set covers only CH1/CH2 for setpoints (p.39), measurement (p.38), the timer (p.41) and the waveform display (p.40), while p.42's status bitmap has no CH3 state bit — which is why reading the state used to hang. Switching CH3 on and off still works and is unchanged (`OUTPut {CH1|CH2|CH3},{ON|OFF}`, p.40). `get_configuration()` omits the unsupported fields instead of raising, and reports the per-output capability flags alongside.
- Oscilloscope: `trigger.level` now raises `FeatureNotSupportedError` where no level command is documented, instead of returning a fabricated `0.0` and silently discarding writes — a `LINE` source on the legacy, LeCroy and Tektronix dialects, and the external input on Tektronix (the 4/5/6 MSO command set documents `TRIGger:A:LEVel:CH<x>` but no AUX form). The modern Siglent dialect is unaffected: its `:TRIGger:EDGE:LEVel` takes no source argument, so nothing is gated there. Code that read `0.0` as a real trigger level was reading a number the library invented. `trigger.get_configuration()` reports such a level as `None` rather than raising, so the rest of the trigger state still comes back.
- Power supply: `OutputSpec` and `PSUCapability` (`scpi_control/psu_models.py`) are now frozen dataclasses. `psu.capabilities` hands back the shared `PSU_MODEL_REGISTRY` entry itself, not a copy, so an in-place mutation (e.g. `PSU_MODEL_REGISTRY["SPD3303X"].has_ovp = True`, a workaround some callers used against the v5.0.0 OVP gate) would change behaviour for every `PowerSupply` instance in the process; it now raises `FrozenInstanceError` instead. Constructing a new `PSUCapability`/`OutputSpec` still works. Callers that were patching a registry entry should build a replacement with `dataclasses.replace(existing, has_ovp=True)` (and register it, if it needs to be looked up by model name) instead of mutating in place.
...
- Power supply: `OutputSpec` carries per-output capability flags (`programmable`, `measurable`, `switchable`, `state_readable`, `supports_timer`, `supports_waveform`) reflecting what the model's manual documents for that output, readable through `psu.capabilities`.
- Oscilloscope: `scope.capabilities.trigger_level_sources` reports which trigger sources have a documented level command on the connected dialect.
...
- `scope.capabilities.trigger_sources` is documented as what the **driver** can send for the connected dialect, not what the attached model physically has — SCPI offers no way to interrogate the latter. Measured on an SDS824X HD (2026-08-04), three cases are accepted with **no error queued** and silently coerced to something else: a channel that is switched off (lands on `LINE`), `EX` on a model with no external trigger input (lands on `LINE`), and `EX5` (a no-op from a channel source; lands on the highest enabled channel from `LINE`). Read `scope.trigger.source` back when the distinction matters. The dialect-wide set is deliberately not restricted, because other modern-dialect scopes do have a working external input.
- `FeatureNotSupportedError` now also subclasses `NotImplementedError`, and the PSU capability gates (tracking, OVP, OCP, timer, waveform) raise it instead of a bare `NotImplementedError`. Existing `except NotImplementedError` handlers keep working; the errors are now also catchable as `SiglentError`.
...
- Mock: the modern mock now answers a trigger-token query with the command table's canonical wire spelling (`SLOPe`, `NORMal`, `RISing`, `HFREJect`) whatever casing the write used, matching an SDS824X HD measured 2026-08-04 — `slope`, `SLOPE` and `sLoPe` all read back `SLOPe` there. It previously echoed the sent casing for mode/slope/coupling and force-uppercased the trigger type, so neither shape matched hardware and no mock test could catch code that depended on the response casing.
...
- Oscilloscope: a screenshot that arrived intact is no longer discarded because the housekeeping behind it failed. The post-capture drain now runs best-effort and logs a warning, where a dropped connection during it used to surface as `RuntimeError: Failed to capture screenshot` with the complete image already in hand. That drain also no longer issues a VISA device clear — a modern `SCDP` capture over `VISAConnection` was issuing a VISA device clear on every single screenshot.
- Oscilloscope: `trigger.level` now works for the `EX` and `EX5` external inputs on the legacy Siglent and LeCroy dialects, where `<trig_source>:TRLV` documents them (RC01020-E01C p.128 / MAUI p.7-33). A `startswith("C")` check had been silently dropping those writes and returning `0.0` for the reads.
- Mock: the PSU mock rejects the undocumented CH3 setpoint, timer and waveform forms with `-224` like real firmware while still accepting `OUTPut CH3,ON`, and the legacy scope mock answers the `EX`/`EX5` `TRLV` forms — so both gates are exercised in CI rather than merely untested.
- PSU: the data logger and the GUI PSU panel now check an output's capabilities before measuring it, instead of calling and catching. For an output with no documented measurement command (an SPD3303X's fixed CH3 rail), the logger writes empty CSV fields where it previously wrote `ERROR`, and the panel shows `---` without polling. Column count and order are unchanged. Code scanning SPD3303X logs for the `ERROR` sentinel on CH3 should treat an empty field as "not measurable on this output".
