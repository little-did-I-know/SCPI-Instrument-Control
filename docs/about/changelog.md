...
- The README hero GIF now tours four examples running with no instrument attached — `basic_usage.py`, `math_channels.py`, `measurement_badges_example.py` and `screen_capture_example.py` — instead of showing a single waveform capture. Every frame's text is captured by running the example as a subprocess at generation time, so the image cannot drift from the code. The file keeps its path (`docs/images/mock-demo.gif`), so no README markup changed.
