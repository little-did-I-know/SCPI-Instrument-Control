...
| `PATCH` | `trigger` | `{mode?, source?, level?, slope?, coupling?}` | State snapshot | 400 invalid value (trigger coupling must be `DC`/`AC`/`HFREJ`/`LFREJ`); 500 if the connected dialect cannot express an otherwise-valid value (e.g. `AC` trigger coupling on Tektronix — see [SCPI Dialects](../user-guide/scpi-dialects.md)); 409 session not accepting jobs |
