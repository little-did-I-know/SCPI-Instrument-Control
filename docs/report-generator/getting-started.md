...
![Annotating a report plot](../images/plot-annotations.gif)
...
`annotations` list (or a `TestSection`'s `fft_annotations` for an FFT plot). Two
runnable examples:

- `examples/report_annotations.py` — all four kinds plus a caption on one waveform
  plot, rendered to a PDF. Start here.
- `examples/report_annotations_advanced.py` — every surface beyond that: all four
  plot types, per-annotation overrides against `PlotStyle` defaults, a region zoom
  inheriting clipped annotations, the sidecar save/restore round-trip, and the anchor
  list the **Annotate...** dialog offers.

The image above is generated by `scripts/media/make_annotation_gif.py`, which
executes the code it renders — regenerate it after changing the annotation API.
